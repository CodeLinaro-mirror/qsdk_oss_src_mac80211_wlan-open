...
int ieee80211_hw_config(struct ieee80211_local *local, int radio_idx, u32 changed)
{
	int ret = 0;
	...
	if (changed && local->open_count) {
		ret = drv_config(local, -1, changed);
		/*
		 * Goal:
					   ...
					   const char *requested_name)
{
	...
	if (requested_name) {
		...
		else if (!strcmp(requested_name, PHY_2GHZ_5GHZ))
			local->wlan_name = "wlan5";
		else
...
int ieee80211_register_hw(struct ieee80211_hw *hw)
{
	...
	if (supp_uhr)
		local->scan_ies_len +=
			3 + sizeof(struct ieee80211_uhr_cap_elem_fixed);
