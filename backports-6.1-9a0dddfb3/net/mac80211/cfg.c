	...
	if (params->uhr_oper) {
		...
		if (params->uhr_cap) {
			...
			link_conf->uhr_config.update_in_tim_interval =
				((params->uhr_cap->fixed.mac.mac_cap[3] &
				  IEEE80211_UHR_MAC_CAP3_UPD_IND_TIM_INTV_LOW) >> 7) |
				((params->uhr_cap->fixed.mac.mac_cap[4] &
				  IEEE80211_UHR_MAC_CAP4_UPD_IND_TIM_INTV_HIGH) << 1);
		}
	}
			     ...
			     unsigned int link_id)
{
	...
	/*
	 * Notify driver that this link is going down
	 * Driver can set flags to optimize peer deletion
	 * Pass wdev->is_netdev_going_down to indicate if entire netdev is going down
	 */
	drv_link_going_down(local, sdata, link_conf, wdev->is_netdev_going_down);
...
static int ieee80211_set_wiphy_params(struct wiphy *wiphy, int radio_idx, u32 changed)
{
	struct ieee80211_local *local = wiphy_priv(wiphy);
	int err = 0;
	...
	if (changed & WIPHY_PARAM_FRAG_THRESHOLD) {
		...
		err = drv_set_frag_threshold(local, radio_idx, wiphy->frag_threshold);
	...
	if ((changed & WIPHY_PARAM_COVERAGE_CLASS) ||
		...
		err = drv_set_coverage_class(local, -1, coverage_class);
...
int ieee80211_dfs_abort_cac(struct wiphy *wiphy,
			    struct wireless_dev *wdev, int link_id)
{
	...
	link_conf = wiphy_dereference(wiphy, sdata->vif.link_conf[link_id]);
	if (!link_conf)
		return -EINVAL;

	chanctx_conf = wiphy_dereference(wiphy, link_conf->chanctx_conf);
	if (chanctx_conf)
		ieee80211_dfs_cac_cancel(local, &chanctx_conf->def);
...
const struct cfg80211_ops mac80211_config_ops = {
	...
	.dfs_process_radar_detection = ieee80211_dfs_process_radar_detection,
	.start_punctured_cac = ieee80211_start_punctured_cac,
