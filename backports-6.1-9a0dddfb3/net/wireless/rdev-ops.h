...
static inline int
rdev_set_default_control_key(struct cfg80211_registered_device *rdev,
			     struct net_device *netdev, int link_id,
			     u8 key_index)
{
	int ret;

	trace_rdev_set_default_control_key(&rdev->wiphy, netdev, link_id,
					   key_index);
	ret = rdev->ops->set_default_control_key(&rdev->wiphy, netdev, link_id,
						 key_index);
	trace_rdev_return_int(&rdev->wiphy, ret);
	return ret;
}
		      ...
		      u32 changed)
{
	...
	if (rdev->ops->set_wiphy_params)
		ret = rdev->ops->set_wiphy_params(&rdev->wiphy, radio_idx, changed);
	trace_rdev_return_int(&rdev->wiphy, ret);
	return ret;
}
				    ...
				    unsigned int link_id, int *dbm)
{
	...
	ret = rdev->ops->get_tx_power(&rdev->wiphy, wdev, radio_idx, link_id, dbm);
