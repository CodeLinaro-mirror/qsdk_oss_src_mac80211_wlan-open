	...
	for (idx = 0; idx < ag->num_devices; idx++) {
		...
		for (i = 0; i < partner_ab->num_radios; i++) {
			...
			for (k = 0; k < cookie_idx[pdv_id]; k++) {
				...
				ret = ath12k_peer_delete(ar, vid, addr, false,
				       ...
				       struct ath12k_wmi_peer_assoc_arg *arg)
{
	 ...
	 */
	if (!sta->epp_peer && (ar->supports_6ghz || rsnie || wpaie || arsta->is_bridge_peer)) {
					     ...
					     struct ath12k_link_vif *arvif)
{
	...
	if (ahvif->vdev_type == WMI_VDEV_TYPE_AP) {
		ret = ath12k_peer_delete(ar, arvif->vdev_id, arvif->bssid, false, 0);
		if (ret)
					 ...
					 struct ath12k_sta *ahsta)
{
	...
	ath12k_peer_mlo_link_peers_delete(ahvif, ahsta, peer_del_all, link_going_down);
...
err_peer_del:
	if (ahvif->vdev_type == WMI_VDEV_TYPE_AP) {
		fbret = ath12k_peer_delete(ar, arvif->vdev_id, link_addr, false, 0);
		if (fbret) {
