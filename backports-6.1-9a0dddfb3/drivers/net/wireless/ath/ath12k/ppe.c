					...
					struct ath12k_vlan_iface *vlan_iface, int link_id)
{
	...
	ath12k_dbg(ab, ATH12K_DBG_PPE,
			"PPEDS vdev attach success soc_idx %d ds_node_id %d "
			"vdev_id %d vpnum %d ppe_vp_profile_idx %d ppe_vp_tbl_idx %d "
			"to_fw %d int_pri %d prec_en %d search_idx_reg_num %d\n",
				 ...
				 struct ieee80211_vif *vif)
{
	...
	ath12k_dbg(ab, ATH12K_DBG_PPE,
			"PPEDS vdev attach success soc_idx %d ds_node_id %d vdev_id %d "
			"vpnum %d ppe_vp_profile_idx %d ppe_vp_tbl_idx %d to_fw %d "
			"int_pri %d prec_en %d search_idx_reg_num %d\n",
			ab->dp->ppe.ppeds_soc_idx, ab->dp->ppe.ds_node_id, vdev_id,
			vp_num, ppe_vp_profile_idx, ppe_vp_tbl_idx, vp_profile->to_fw,
			vp_profile->use_ppe_int_pri, vp_profile->drop_prec_enable,
			vp_profile->search_idx_reg_num);
