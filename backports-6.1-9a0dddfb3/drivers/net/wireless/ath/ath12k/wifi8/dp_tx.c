			...
			u32 qos_nw_delay)
{
	...
	tx_desc->skb = skb;
	tx_desc->mac_id = dp_pdev->hw_link_id;
					  ...
					  struct ath12k_tx_sw_metadata *sw_metadata)
{
	...
	dp_pdev = ath12k_dp_hw_grp_to_dp_pdev(dp->dp_hw_grp, sw_metadata->mac_id);
	if (!dp_pdev) {
		rcu_read_unlock();
		return;
	}
				       ...
				       struct sk_buff *skb, struct ath12k_link_sta *arsta)
{
	...
	return ath12k_wifi8_dp_tx(dp_pdev, arvif, skb, false, 0,
				  false, arsta, ring_id, 0);
}
...
int ath12k_wifi8_ppeds_tx_completion_handler(struct ath12k_base *ab, int budget)
{
	...
	while (likely(valid_entries--)) {
		...
		if (unlikely(buf_rel_source == HAL_WBM_REL_SRC_MODULE_TQM)) {
