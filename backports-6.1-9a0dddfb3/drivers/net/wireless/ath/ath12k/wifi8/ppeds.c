...
int ath12k_wifi8_dp_ppeds_register_soc(struct ath12k_dp *dp, struct dp_ppe_ds_idxs *idx)
{
	...
	if (!(ath12k_ppeds_reo2ppe_rings_max) ||
		(ath12k_ppeds_reo2ppe_rings_max > REO2PPE_MAX_RINGS) ||
		!(ath12k_ppeds_ppe2tcl_rings_max) ||
		(ath12k_ppeds_ppe2tcl_rings_max > PPE2TCL_MAX_RINGS)) {
	...
	if (dp->ppe.hw_buff_mgmt) {
		...
		hbm_ring_info->ppe2wbm_num_desc = DP_PPE2WBM_REFILL_RING_SIZE;
	...
	if (reg_info.wifi8_cfg.data_ring_auto_index_en) {
		...
		for (ring_idx = 0; ring_idx < ppe2tcl_rings_max; ring_idx++) {
			ppe2tcl_irq_type = (ring_idx * PPEDS_IRQ_NEXT_RING) +
						PPEDS_IRQ_PPE2TCL;
		...
		for (ring_idx = 0; ring_idx < reo2ppe_rings_max; ring_idx++) {
			reo2ppe_irq_type =
				(ring_idx * PPEDS_IRQ_NEXT_RING) + PPEDS_IRQ_REO2PPE;
...
skip_ppeds_dp_srng_ring_alloc:
	...
	for (idx = 0; idx < ath12k_ppeds_reo2ppe_rings_max; idx++) {
		/* TODO:Remove if EDMA now supports starting at index 0 */
		/* TODO:Use ring_id fetched from ppe for avoiding edma hang during SSR */
